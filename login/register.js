...
const verification =
  verificationTemplate.content.querySelector("div.form.verify");
...
form.onsubmit = async (e) => {
  ...
  const response = await fetch(
    `${location.protocol}//${location.host}/account/register`,
    options
  );
...
function createVerificationDiv(email) {
  ...
  const verificationCodeInput = verification.querySelector(
    ...
    let timer = setInterval(() => {
      ...
      if (secs === 0) {
        ...
        resendBtn.onclick = () => {
          createTimer();
          fetch(
            `${location.protocol}//${location.host}/account/verify/resend`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
